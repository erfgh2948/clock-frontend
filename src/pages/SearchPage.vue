    ...
    <div class="wine-container"> 
      ...
      <div class="benner">
        ...
        <div class="search-filter">
          <input type="text" placeholder="와인검색" />
        ...
        <div class="search-container">
          ...
          <div class="ranking-container">
            ...
            <div class="tab_menu">
              <ul class="list">
                <li class="is_on">
                  <a href="#tab1" class="btn" :class="{on:topList==1}" @click="select1">TOP 10</a>
                </li>
                <li>
                  <a href="#tab2" class="btn" :class="{on:topList==2}" @click="select2">TOP 20</a>
                </li>
                <li>
                  <a href="#tab3" class="btn" :class="{on:topList==3}" @click="select3">TOP 30</a>
                </li>
              </ul>
              <!-- 1~10 -->
              <div v-if="topList==1">
...
export default{
  ...
  setup(){   
    ...
    axios.get("https://port-0-wine-backend-4uvg2mledushse.sel3.cloudtype.app/api/wineList")
    .then(({data})=>{
      statee.items=data;
    ...
    return {state,CheckBox, testStoreGetters, thisValue,statee,topList,select1,select2,select3}
  },
...
@media screen and (min-width:600px) {
  ...
  /* 배너 및 검색버튼 */
  .benner-img {
    width:80%; height:60%; margin:15px auto;
    background:url('@/assets/wine.gif') no-repeat center center;
    background-size:cover;
  }
  .benner input {
    display:block;
    width:80%; height:30px; margin:25px auto; padding-left:15px;
    color:darkgrey;
...
.rankListLinkContainer {
  ...
  .search-container{    
    height:100%;
    padding:0 40px 40px; border-left:1px solid #333;
  }
  .filter {
  ...
  .filter span {display:flex;}
  .filter-img {
    width:16px; height:17px;
    background:url('@/assets/ico-filter.png');
  }
  .filter p {margin:0 auto;}
...
@media screen and (max-width:599px) {
  ...
  /* 그리드 아이템 속성 지정 */
  .benner {grid-row:1 / 2; grid-column: 1 / 3;}
  .allwine {grid-row:2 / 3;  grid-column: 1 / 3;}
  .search {grid-row: 2 / 3; grid-column: 1 / 3;}
  ...
  .search-filter {
    display:flex;
    width:80%; height:30px; margin:20px auto;
   }
  .search-filter input {
    display:block;
    width:90%; height:30px; margin:0; padding-left:15px;
    color:darkgrey;
    border:none; border-radius:3px; background:gainsboro;
  }
  .search-filter input:focus {outline: 1px solid #d50000;}
